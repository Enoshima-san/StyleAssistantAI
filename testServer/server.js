...
app.post('/saveAnswer', authenticateToken, async (request, response) => {
    ...
    console.log('=== Данные для сохранения образа ===');
    console.log('User ID:', userId);
    console.log('AI Response:', data.aiResponse);
    console.log('AI Prompt:', data.aiPrompt);
        ...
        db.run('INSERT INTO Образы (ID_Образа, ID_Пользователя, Название, Параметры_Генерации, Дата_Создания) VALUES (?, ?, ?, ?, ?)',
            ...
            (err) => {
                ...
                if (data.productIds && data.productIds.length > 0) {
                    ...
                    // Если нет товаров для сохранения
                    if (total === 0) {
                    ...
                    data.productIds.forEach((productId, index) => {
                        if (!productId || productId === 'undefined' || productId === 'null') {
                            console.log(`Пропущен пустой productId на позиции ${index + 1}`);
                            completed++;
                            if (completed === total) {
                        ...
                        // ИСПРАВЛЕНИЕ: Убираем ID_Элемента - он autoincrement
                        db.run('INSERT INTO Элементы_Образа (ID_Образа, ID_Товара, Позиция) VALUES (?, ?, ?)',
                            ...
                            (err) => {
                                if (err) {
                                    ...
                                    console.error('Параметры:', [outfitId, productId, index + 1]);
...
app.get('/catalog-products', authenticateToken, async (req, res) => {
    const sqlQuery = `
        SELECT
            т.ID_Товара,
            т.Название AS Название_Товара,
        ...
        FROM Товары т
                 LEFT JOIN Характеристики_Товаров х ON т.ID_Товара = х.ID_Товара
        WHERE х.Активен = 1
          AND х.Ссылка_Изображение IS NOT NULL
          AND х.Ссылка_Изображение != ''
        ORDER BY RANDOM()
        LIMIT 20
    `;
    ...
    db.all(sqlQuery, [], (err, rows) => {
        if (err) {
            console.error('Ошибка при получении товаров каталога:', err.message);
            return res.status(500).json({ error: 'Ошибка базы данных' });
        }
...
app.post('/save-catalog-outfit', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const data = req.body;
    ...
    try {
        // Проверяем, не существует ли уже такой образ
        db.get('SELECT ID_Образа FROM Образы WHERE ID_Пользователя = ? AND Название = ?',
            [userId, data.outfitName || 'Образ из каталога'],
            (err, existingOutfit) => {
                if (err) {
                ...
                if (existingOutfit) {
                    console.log('Образ уже существует');
                    return res.status(200).json({
                        message: 'Образ уже существует в избранном',
                        outfitId: existingOutfit.ID_Образа
                    });
                }

                // Сохраняем образ из каталога
                db.run('INSERT INTO Образы (ID_Образа, ID_Пользователя, Название, Параметры_Генерации, Дата_Создания) VALUES (?, ?, ?, ?, ?)',
                    [outfitId, userId, data.outfitName || 'Образ из каталога', 'catalog_outfit', currentDate],
                    (err) => {
                        if (err) {
                            console.error('Ошибка сохранения образа из каталога:', err.message);
                        ...
                        if (data.productIds && data.productIds.length > 0) {
                            ...
                            data.productIds.forEach((productId, index) => {
                                ...
                                db.run('INSERT INTO Элементы_Образа (ID_Образа, ID_Товара, Позиция) VALUES (?, ?, ?)',
                                    ...
                                    (err) => {
                                        if (err) {
                                            console.error('Ошибка сохранения элемента из каталога:', err.message);
                                        } else {
                                            console.log(`Сохранен товар ${index + 1}/${total}: ${productId}`);
                                        }
                                        ...
                                        if (completed === total) {
                                            res.status(201).json({
                                                message: 'Образ из каталога и товары сохранены',
                                                outfitId: outfitId
                                            });
                                        }
                                    }
                            ...
                            console.log('Нет товаров для сохранения из каталога');
                            res.status(201).json({
                                message: 'Образ из каталога сохранен без товаров',
                                outfitId: outfitId
                            });
                        }
                    }
        ...
        console.error('Ошибка при сохранении образа из каталога:', error);
        res.status(500).json({
            error: 'Внутренняя ошибка сервера',
            details: error.message
        });
